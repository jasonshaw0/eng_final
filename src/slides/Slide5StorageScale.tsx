        ...
        <div className="flex-1 flex flex-col justify-center">
          <div className="bg-white rounded-2xl border border-border p-8 card-shadow h-full flex flex-col justify-center">
            ...
            <div className="space-y-3 bg-bg-surface rounded-xl p-5 border border-border">
              {slide.bullets.map((b, i) => (<div key={i} className="flex gap-3 items-start"><span className="w-2 h-2 rounded-full bg-accent-purple mt-2.5 shrink-0" /><span className="text-base text-text-secondary leading-relaxed">{b}</span></div>))}
            </div>
          </div>
        </div>
